...
const GROUP_LABELS = { PM:'Pre-Medical', PE:'Pre-Engineering', ICS:'ICS', ICOM:'I.Com' };

// key: internal id, sheetName: exact name expected in workbook, group -> subject set
const SECTION_DEFS = [
...
/* ---- 026_custom-report-any-section-subject-status-combo.js ---- */

/* ===================== CUSTOM REPORT (any section x subject x status combination) =====================
   Lets the user print a physical-record report for any permutation of
   Section (or All Sections) x Subject (or All Subjects) x Status
   (Improving / Declining / At Risk / Absent / All). Accessible from the
   masthead on every page, independent of which report screen is open. */
const STATUS_LABELS_CR = {all:'All Students', improving:'Improving', declining:'Declining', atrisk:'At Risk (Red Zone)', absent:'Absent'};

function populateCustomReportSections(){
  const sel = document.getElementById('crSection');
  const prev = sel.value;
  sel.innerHTML = '<option value="all">All Sections</option>' +
    SECTION_DEFS.map(d=>`<option value="${d.key}">${escapeHtml(d.label)}</option>`).join('');
  sel.value = prev && (prev==='all' || SECTION_BY_KEY[prev]) ? prev : 'all';
}
function populateCustomReportSubjects(){
  ...
  if(sectionKey==='all'){
    ...
    subjects = (SECTION_BY_KEY[sectionKey]||{subjects:[]}).subjects;
  }
  sel.innerHTML = '<option value="all">All Subjects</option>' +
    subjects.map(s=>`<option value="${escapeHtml(s)}">${escapeHtml(s)}</option>`).join('');
  sel.value = subjects.includes(prev) ? prev : 'all';
}

// Returns [{sectionLabel, subject, name, roll, latest, statusLabel}] for the
// chosen section/subject/status combination. One row per matching
// student x subject (a student can appear more than once under "All
// Subjects" if they match in several subjects).
function getCustomReportRows(sectionKey, subject, statusKey){
  const defsToUse = sectionKey==='all' ? SECTION_DEFS : (SECTION_BY_KEY[sectionKey] ? [SECTION_BY_KEY[sectionKey]] : []);
  const rows = [];
  defsToUse.forEach(def=>{
    const store = workspace.sections[def.key];
    if(!store) return;
    const subjectsToUse = subject==='all' ? def.subjects : (def.subjects.includes(subject) ? [subject] : []);
    subjectsToUse.forEach(subj=>{
      store.students.forEach(st=>{
        const arr = (st.tests||{})[subj] || [];
        const t = arr.length ? arr[arr.length-1] : null;
        let match = false, statusLabel = '—';
        if(statusKey==='all'){
          match = !!t;
          if(t) statusLabel = t.absent ? 'Absent' : (ZONE_LABEL[zoneOf(t.percent,false)]||'—');
        } else if(statusKey==='improving'){
          const c = classifyTransition(arr);
          match = !!(c && c.key==='improved');
          statusLabel = 'Improving';
        } else if(statusKey==='declining'){
          const c = classifyTransition(arr);
          match = !!(c && c.key==='declined');
          statusLabel = 'Declining';
        } else if(statusKey==='atrisk'){
          match = !!(t && !t.absent && t.percent!=null && zoneOf(t.percent,false)==='red');
          statusLabel = 'At Risk';
        ...
        rows.push({
          ...
          name: st.name,
          roll: st.rollNo || '',
          latest: t ? (t.absent ? 'Absent' : (t.percent!=null ? `${t.percent}%` : '—')) : '—',
          statusLabel
        });
      });
    });
...
function updateCustomReportPreview(){
  ...
  const statusKey = document.getElementById('crStatus').value;
  const rows = getCustomReportRows(sectionKey, subject, statusKey);
  document.getElementById('crPreviewCount').textContent = `${rows.length} matching record${rows.length===1?'':'s'}`;
}
...
function generateCustomReportPDF(){
  ...
  const sectionLabel = sectionKey==='all' ? 'All Sections' : (SECTION_BY_KEY[sectionKey]||{label:sectionKey}).label;
  const title = `Custom Report — ${sectionLabel} · ${subject==='all'?'All Subjects':subject} · ${STATUS_LABELS_CR[statusKey]||statusKey}`;
  const subtitle = `${rows.length} matching record${rows.length===1?'':'s'}`;

  if(!rows.length){
    printReport(title, subtitle, `<div style="padding:20px;color:#777;">No students match this combination.</div>`);
    document.getElementById('customReportOverlay').classList.remove('open');
    return;
  }

  const bySection = {};
  rows.forEach(r=>{ (bySection[r.sectionLabel] = bySection[r.sectionLabel] || []).push(r); });

  let body = '';
  Object.keys(bySection).sort().forEach(sl=>{
    const list = bySection[sl];
    body += reportSectionHtml(sl, reportTableHtml(['Name','Roll','Subject','Latest %','Status'],
      list.map(r=>[r.name, r.roll, r.subject, r.latest, r.statusLabel])));
  });

  printReport(title, subtitle, body);
  document.getElementById('customReportOverlay').classList.remove('open');
}

document.getElementById('customReportBtn').addEventListener('click', ()=>{
  populateCustomReportSections();
  populateCustomReportSubjects();
  updateCustomReportPreview();
  document.getElementById('customReportOverlay').classList.add('open');
});
document.getElementById('crCancelBtn').addEventListener('click', ()=>{
  document.getElementById('customReportOverlay').classList.remove('open');
});
document.getElementById('customReportOverlay').addEventListener('click', (e)=>{
  if(e.target.id === 'customReportOverlay') document.getElementById('customReportOverlay').classList.remove('open');
});
document.getElementById('crSection').addEventListener('change', ()=>{
  populateCustomReportSubjects();
  updateCustomReportPreview();
});
document.getElementById('crSubject').addEventListener('change', updateCustomReportPreview);
document.getElementById('crStatus').addEventListener('change', updateCustomReportPreview);
document.getElementById('crPrintBtn').addEventListener('click', generateCustomReportPDF);
